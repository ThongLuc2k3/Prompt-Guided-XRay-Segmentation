...
# def visualize_3_scenarios(model, img_name):
#     img_path  = os.path.join(TEST_IMAGE_DIR, img_name)
#     json_path = os.path.join(TEST_JSON_DIR, os.path.splitext(img_name)[0] + '.json')
#     if not os.path.exists(img_path):
#         print(f"Không tìm thấy: {img_path}")
...
#     print(f"\nVisualize 3 kịch bản: {img_name} – Vùng xương thực tế (Shape index: {target_idx})")
...
#     ds_helper = BTXRD_Dataset(TEST_IMAGE_DIR, TEST_JSON_DIR, img_size=IMG_SIZE, is_train=False)
#     r_fixed = (ds_helper.zoom_ratio[0] + ds_helper.zoom_ratio[1]) / 2
...
#     fig, axes = plt.subplots(3, 5, figsize=(24, 14))
#     fig.suptitle(f"Exp B – 3 kịch bản prompt | {img_name} (Shape {target_idx})", fontsize=15, fontweight='bold')
...
#             # Tính tâm dự đoán hình học cho từng kịch bản
#             if pred_np.sum() > 0:
#                 ys_p, xs_p = np.where(pred_np == 1)
#                 cx_p, cy_p = xs_p.mean(), ys_p.mean()
#             else:
#                 cx_p, cy_p = None, None
...
#             # Cột 0: Ảnh gốc
#             axes[row][0].imshow(img_np, cmap='gray')
#             axes[row][0].set_ylabel(scenario_name, fontsize=11, fontweight='bold', rotation=0, labelpad=60, va='center')

#             # Cột 1: Prompt Heatmap
#             axes[row][1].imshow(img_np, cmap='gray')
#             axes[row][1].imshow(np.ma.masked_where(pm_np < 0.05, pm_np), cmap='magma', alpha=0.6)

#             # Cột 2: Ground Truth dạng mảng xanh trong suốt + Chấm tâm
#             axes[row][2].imshow(img_np, cmap='gray')
#             green_overlay = np.zeros((*gt_np.shape, 4))
#             green_overlay[gt_np == 1] = [0, 1, 0, 0.35]
#             axes[row][2].imshow(green_overlay)
#             if gt_np.max() > 0:
...
#             # Cột 3: Dự đoán dạng mảng đỏ trong suốt + Chấm tâm
#             axes[row][3].imshow(img_np, cmap='gray')
#             red_overlay = np.zeros((*pred_np.shape, 4))
#             red_overlay[pred_np == 1] = [1, 0, 0, 0.35]
#             axes[row][3].imshow(red_overlay)
#             if cx_p is not None:
#                 axes[row][3].plot(cx_p, cy_p, marker='o', color='red', markersize=8, alpha=1.0, markeredgecolor='white')

#             # Cột 4: Nét cắt biên so sánh + Hiện cả 2 tâm để đo độ lệch trực quan
#             axes[row][4].imshow(img_np, cmap='gray')
#             if gt_np.max() > 0:   axes[row][4].contour(gt_np,   [0.5], colors='lime', linewidths=2)
#             if pred_np.max() > 0: axes[row][4].contour(pred_np, [0.5], colors='red',  linewidths=2, linestyles='--')
#             if cx_gt is not None:
#                 axes[row][4].plot(cx_gt, cy_gt, marker='o', color='lime', markersize=7, alpha=1.0, markeredgecolor='black')
#             if cx_p is not None:
#                 axes[row][4].plot(cx_p, cy_p, marker='o', color='red', markersize=7, alpha=1.0, markeredgecolor='white')
                
#             axes[row][4].set_title(f"Dice:{dice:.3f} | HD95:{hd95:.1f}px | CBL:{cbl:.3f}", fontsize=9, fontweight='bold')
...
#     print(f"Visualize inference check: {img_name} – (Shape {target_idx})")
#     points = np.array(target_shape['points'])
...
#     fig, axes = plt.subplots(2, 5, figsize=(25, 10))
#     fig.suptitle(f"Cải tiến inference – phát hiện prompt sai | {img_name}", fontsize=14, fontweight='bold')
...
#     # Danh sách các ảnh cần chạy kiểm thử
#     list_images = [
#         "IMG001768.png",
#         "IMG001538.png",
#         "IMG001100.png",
...
#         visualize_3_scenarios(trained_model, img_name=img)
#         visualize_inference_check(trained_model, img_name=img)