...
# Đổi thành tên ảnh + shape index muốn visualize
...
def visualize(model, img_name=VIZ_IMG_NAME, shape_idx=VIZ_SHAPE_IDX):
    print(f"\nVisualize: {img_name} – shape {shape_idx}")
    ...
    fig.suptitle(f"Exp A – Zoom-out | {img_name} shape {shape_idx}", fontsize=14, fontweight='bold')
    ...
    axes[2].imshow(np.ma.masked_where(gt_np == 0, gt_np), cmap='Greens', alpha=0.5)
    axes[3].imshow(img_np, cmap='gray')
    axes[3].imshow(np.ma.masked_where(pred_np == 0, pred_np), cmap='Reds', alpha=0.5)
    ...
    plt.tight_layout()
    plt.savefig(f"result_expA_{os.path.splitext(img_name)[0]}_shape{shape_idx}.png",
